...
function War_WugongHurtLife(enemyid, wugong, level, ang, x, y)
	...
	--除却四相，40%几率免疫本次攻击造成的内伤/封穴/冰封/灼烧
	if ChuQueSX(eid) and JLSD(20,60,eid) then
		WAR.CQSX = 1
	...
	if WAR.NGJL == 96 or (Curr_NG(pid, 108) and PersonKF(pid, 96)) then
		...
		if not WAR.NGJL == 96 then	--易筋经触发的罗汉伏魔，只有一半效果
			nlmod = nlmod /2
		end
	...
	if WAR.TJAY == 3 then
		...
		if PersonKF(eid, 171) or JLSD(40, 65, eid) then
			...
			Set_Eff_Text(enemyid,"特效文字1","太极奥义");
		end
	end
	...
	if (wugong == 61 or wugong == 65 or wugong == 66) and JuHuoLY(pid) and WAR.LQZ[pid] == 100 and DWPD() then
		WAR.JHLY[eid] = 10
		WAR.Person[enemyid]["特效动画"] = 112
	end
	
	--无酒不欢：利刃寒锋，修罗+阴风+沧溟，暴怒造成冻结效果
	if (wugong == 58 or wugong == 174 or wugong == 153) and LiRenHF(pid) and WAR.LQZ[pid] == 100 and DWPD() then
		WAR.LRHF[eid] = 10
	...
	--狄云赤心连城追加连击
	if match_ID(pid, 37) and pid == 0 and hurt < 200 then
		WAR.CXLC = 1
	end
	...
	elseif PersonKF(pid, 163) then
		if WAR.XHSJ < 100 then
			WAR.XHSJ = WAR.XHSJ + limitX(math.modf(hurt * 0.05),0,100)
			if WAR.XHSJ > 100 then
				WAR.XHSJ = 100
			end
		end
	...
	--除却四相免疫封穴
	--主运瞬息千里，暴怒状态不会被封穴
	if Curr_NG(eid, 104) == false and (Curr_QG(eid,150) and WAR.LQZ[eid]==100)==false and DWPD() and 50 <= hurt and (WAR.BFX == 1 or JLSD(30, 60, pid)) and WAR.CQSX == 0 then
		--无酒不欢：使用分段函数
		local fxz = 1;
	...
	--防御方带6级金丝免疫流血
	if hurt > 30 and DWPD() and (JY.Person[eid]["防具"] == 239 and JY.Thing[239]["装备等级"] == 6) == false and (WAR.L_TLD == 1 or WAR.BLX == 1 or WAR.GCTJ == 1 or JLSD(30, 60, pid)) then
	...
	if not (match_ID(eid, 129) and WAR.CYZX[eid] ~= nil and WAR.BDQS > 0) then
		...
		if WAR.TXZZ == 1 and WAR.LQZ[eid] ~= nil and WAR.LQZ[eid] == 100 and WAR.DZXY ~= 1 and JLSD(10, 60 + JY.Base["天书数量"] + math.modf(JY.Person[pid]["实战"]/50), pid) then
	...
	--紫气天罗，杀人引爆毒素
	if JY.Person[eid]["生命"] <= 0 and ZiqiTL(pid) and DWPD() then
		local dam = JY.Person[eid]["中毒程度"]*4
		for i = 0, WAR.PersonNum - 1 do
	...
	if hurt > 30 and DWPD() then
		poisonnum = math.modf((poisonnum - JY.Person[eid]["抗毒能力"] - JY.Person[eid]["内力"] / 50) / 4)
		if poisonnum < 0 then
			poisonnum = 0
		end
		--太玄之轻免疫中毒
