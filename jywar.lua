...
function War_WugongHurtLife(enemyid, wugong)
	...
	--袈裟伏魔功效果
	if Match_wugong(pid, wugong, 125) and IsStrike(pid) then
		ForceDirection(enemyid)
    end
	--七弦无形剑效果
	...
	--拈花指效果
    if Match_wugong(pid, wugong, 137) and IsStrike(pid) and (WAR.LQZ[pid] or 0) > 0 then
		WAR.Person[WAR.CurID]["生命点数"] = (WAR.Person[WAR.CurID]["生命点数"] or 0) + AddPersonAttrib(pid, "生命", WAR.LQZ[pid]);
		Cls();
		War_Show_Count(WAR.CurID, "拈花微笑")
		WAR.LQZ[pid] = 0
    end
	--鹰爪功效果
	...
	--七伤拳效果
    if Match_wugong(pid, wugong, 23) and IsStrike(pid) then
		AddInternalDamage(pid, 7)
    end
	--铁指诀效果
	...
	--神门十三剑效果
    if Match_wugong(pid, wugong, 79) and IsStrike(pid) then
		AddFreeze(eid, (WAR.FXDS[eid] or 0))
    end
	--铁掌效果
    ...
    if Match_wugong(pid, wugong, 39) and (IsStandStill(pid) or IsCounter(pid)) then
		WAR.Person[WAR.CurID]["生命点数"] = (WAR.Person[WAR.CurID]["生命点数"] or 0)
		 + AddPersonAttrib(pid, "生命", (WAR.Shield[pid] or 0));
		Cls();
    ...
    if WugongCanHeal(pid, wugong) then
		isHeal = true
	--两仪剑法 反两仪刀法 狮子吼 慈悲刀法 拈花指无误伤
	elseif Match_wugong(pid, wugong, 37) or Match_wugong(pid, wugong, 60) or Match_wugong(pid, wugong, 33)
	or Match_wugong(pid, wugong, 57) or Match_wugong(pid, wugong, 137) then
		noDamage = true
    end
	end
	...
	if JY.Person[eid]["生命"] <= 0 then
		local overkill = -JY.Person[eid]["生命"]
		--神门十三剑效果
		if overkill > 0 and Match_wugong(pid, wugong, 79) then
			AddShield(pid, overkill)
		end
		--疯魔杖法效果
		if Match_wugong(pid, wugong, 86) then
			AddRage(pid, 10)
			WAR.FMZF[pid] = (WAR.FMZF[pid] or 0) + 1
		end
		--血刀大法
		if Match_wugong(pid, wugong, 63) then
			JY.Person[pid]["生命最大值"] = JY.Person[pid]["生命最大值"] + 10
			if IsBackstab(WAR.CurID, enemyid) then
				JY.Person[pid]["生命最大值"] = JY.Person[pid]["生命最大值"] + 10
			end
		end
	...
	--慈悲刀法效果
	if JY.Person[eid]["生命"] == 0 and Match_wugong(pid, wugong, 57) then
		WAR.Person[enemyid]["我方"] = WAR.Person[WAR.CurID]["我方"]
		JY.Person[eid]["生命"] = math.min(JY.Person[eid]["冰封程度"] * 5, JY.Person[eid]["生命最大值"])
		WAR.Person[enemyid].Time = math.min(0, WAR.Person[enemyid].Time)
	end

	--拈花指效果
	if JY.Person[eid]["生命"] == 0 and Match_wugong(pid, wugong, 137) then
		WAR.Person[enemyid]["我方"] = WAR.Person[WAR.CurID]["我方"]
		JY.Person[eid]["生命"] = math.min((WAR.FXDS[eid] or 0) * 5, JY.Person[eid]["生命最大值"])
		WAR.Person[enemyid].Time = math.min(0, WAR.Person[enemyid].Time)
	end
	...
	return limitX(hurt, 0, hurt), raged
end
...
function WarShowHead(id)
	...
	--疯魔杖法
	if WAR.FMZF[pid] ~= nil then
		if WAR.Person[id]["我方"] == true then
			lib.LoadPNG(98, 18 * 2 , x1 - size*9- CC.RowPixel, CC.ScreenH - size*2 - CC.RowPixel*2 - (size*2+CC.RowPixel*2)*zt_num, 1)
			DrawString(x1 - size*6- CC.RowPixel, CC.ScreenH - size - CC.RowPixel*3 + 1 - (size*2+CC.RowPixel*2)*zt_num, "疯魔杖法:" .. WAR.FMZF[pid], C_WHITE, size)
		else
			lib.LoadPNG(98, 18 * 2 , x1 + width + CC.RowPixel, CC.RowPixel + 3 + (size*2+CC.RowPixel*2)*zt_num, 1)
			DrawString(x1 + width + size*2 + CC.RowPixel*3, size + 3 + (size*2+CC.RowPixel*2)*zt_num, "疯魔杖法:" .. WAR.FMZF[pid], C_WHITE, size)
		end
		zt_num = zt_num + 1
	end
...
function War_CalculateDamage(pid, eid, wugong)
	...
	if (IsStrike(pid) or dragon) and WAR.LQZ[pid] ~= nil then
		...
		--拈花指
		if Match_wugong(pid, wugong, 137) and IsStrike(pid) then
			raged = 0
		end
		dmg = dmg + WAR.LQZ[pid]
		if dragon then
...
function War_Fight_Sub(id, wugongnum, x, y)
	...
	elseif wugongnum < 1000 then
		...
		WarDrawMap(0)
		local fj = JY.Wugong[wugong]["名称"] .. "-合击"		
	...
	--红色 两仪剑法
	if Match_wugong(pid, wugong, 37) and (GetBaguaEffect(id) == 2 or GetBaguaEffect(id) == 9) then
		if a1 == 0 then
			a1 = 3
		end
		a2 = a2 + 1
	...
	--疯魔杖法
	if Match_wugong(pid, wugong, 86) and (WAR.FMZF[pid] or 0) > 0 then
		if a1 == 0 then
			a1 = 3
		end
		a2 = a2 + WAR.FMZF[pid]
  ...
  while WAR.ACT <= fightnum do
	...
	if GetWarMap(WAR.Person[id]["坐标X"], WAR.Person[id]["坐标Y"], 6) == 2
	or (Match_wugong(pid, wugong, 37) and GetBaguaEffect(id) == 9) then
		WAR.Person[id]["特效动画"] = 6
		if WAR.Person[id]["特效文字3"] ~= nil then
	...
	local function checkSquare(i, j)
		...
		--拈花指
		if Match_wugong(pid, wugong, 137) then
			local i9, j9 = i - WAR.Person[WAR.CurID]["坐标X"], j - WAR.Person[WAR.CurID]["坐标Y"]
			if i9 > 1 then i9 = 1 end
			if i9 < -1 then i9 = -1 end
			if j9 > 1 then j9 = 1 end
		...
		--袈裟伏魔功
		if Match_wugong(pid, wugong, 125) then
			local i9, j9 = i - WAR.Person[WAR.CurID]["坐标X"], j - WAR.Person[WAR.CurID]["坐标Y"]
			if i9 > 1 then i9 = 1 end
			if i9 < -1 then i9 = -1 end
			if j9 > 1 then j9 = 1 end
	...
	--新合击
	if wife > 0 and #wifeset > 0 then
		for i = 1, #wifeset do
			WifeAttack(wifeset[i], wife, x, y)
		end
	end
...
function WifeAttack(newid, kongfuid, x, y)
	local tmp = WAR.CurID
	WAR.CurID = newid
	WAR.WIFE = 1
	War_Fight_Sub(newid, kongfuid + 1000, x, y)
	WAR.CurID = tmp
	WAR.WIFE = 0
end
  ...
  while true do
    ...
    elseif (key == VK_SPACE or key == VK_RETURN) then
		...
		if IsFullForce(pid, wugong) and not IsStandStill(pid) then
			--碧海潮生曲
			if not Match_wugong(pid, wugong, 124) or GetBaguaEffect(WAR.CurID) <= 0 then
		...
		--九阴白骨爪 多罗叶指 天山六阳掌 一阳指 袈裟伏魔功 拈花指可以打空格
		if WAR.Person[GetWarMap(x, y, 2)] == nil and wugong ~= 11 and
		wugong ~= 132 and wugong ~= 8 and wugong ~= 17 and wugong ~= 125 and
		wugong ~= 137 then
			DrawStrBoxWaitKey("需指定敌人为目标", C_WHITE, CC.DefaultFont)
			isValid = false
		end
		if WAR.Person[GetWarMap(x, y, 2)] ~= nil and WAR.Person[GetWarMap(x, y, 2)]["我方"] == true and not WugongCanAlly(pid, wugong) then
			DrawStrBoxWaitKey("需指定敌人为目标", C_WHITE, CC.DefaultFont)
			isValid = false
		end
...
function GetBaguaEffect(warid)
	return GetWarMap(WAR.Person[warid]["坐标X"], WAR.Person[warid]["坐标Y"], 6)
end
--黄蓉：奇门遁甲
    ...
    for p = 0, WAR.PersonNum - 1 do
        ...
        if WAR.ZYHB == 1 then
			...
			local function TurnEndReal()
				...
				--八卦掌法
				if GetBaguaEffect(WAR.CurID) > 0 and MatchStyle(id, 133) then
					for j = 0, WAR.PersonNum - 1 do
						if RealJL(WAR.CurID, j) <= 1 and WAR.Person[j]["我方"] ~= WAR.Person[WAR.CurID]["我方"] and JY.Person[WAR.Person[j]["人物编号"]]["生命"] > 0 then
							AddStun(WAR.Person[j]["人物编号"], 10)
						end
					end
					if WAR.Person[p]["我方"] == true then
						kfmoveAferwards(true)
				...
				if GetWarMap(WAR.Person[WAR.CurID]["坐标X"], WAR.Person[WAR.CurID]["坐标Y"], 6) == 3 and JY.Person[id]["生命"] > 0 then
					heal_factor = heal_factor + 0.16
				end
				if MatchStyle(pid, 60) and (GetBaguaEffect(id) == 3 or GetBaguaEffect(id) == 9) then
					heal_factor = heal_factor + 0.16
				end
				if heal_factor > 0 then
					local heal_amount = math.modf((JY.Person[id]["生命最大值"] - JY.Person[id]["生命"]) * heal_factor)
